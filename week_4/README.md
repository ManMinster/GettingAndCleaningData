...
This will also help indiciate if the structure of the merged data set in correct later on.
...
The object dataFeaturesNames contains the names of the features which was stored earlier on. This will be used to replace the current variable names in dataFeatures.

data\$subject and data\$activity both contain one variable, "subject" and "activity" repectively.
...
Find all variables containing the string 'mean' or 'std' and store it in a variable subdataFeaturesNames.

```r
subdataFeaturesNames <- dataFeaturesNames$V2[grep("mean\\(\\)|std\\(\\)", dataFeaturesNames$V2)]
```

Subset columns listed in subdataFeaturesNames, inluding the 'activity' and 'subject' columns.
...
Replace the activity type which is currently denoted by number 1,2,3,4,5,6 with
the corresponding activity name listed in the activity_labels.txt file.
...
The variable in the subject, activity and the acitivy names have already been appropriately labeled. see 2. However, the names of the features will be replaced with full descriptive variables. 
...
Use the aggregate function add the subject and activity values. Find the average by passing in the mean function.

Export the data table to a text file using the write.table function.

### Note: The instructions tell us to use `row.name=FALSE`, therefore the column names will be of the original,
but the first row contain the descriptibe column names taken from the `features.txt` file.
